...
document.querySelectorAll(".toggle-features").forEach((btn) => {
  ...
  // 🔁 Enlaces internos: scroll automático al destino
...
function triggerSearch() {
  ...
  if (matches.length === 0) {
   ...
   resultsContainer.innerHTML = similar
    ? `<p class="faq-no-results">No se encontraron coincidencias exactas.<br>¿Quizás quisiste decir: <strong>${similar.q}</strong>?<p>`
  ...
  card.innerHTML = `
    <div class="faq-question" data-id="${id}" id="${id}">${q}</div>
